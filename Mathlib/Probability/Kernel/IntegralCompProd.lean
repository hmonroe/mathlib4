...
`α` to `β` and `η` is an s-finite kernel from `β × α` to `γ`, we can form their composition-product
`κ ⊗ₖ η : kernel α (γ × β)`. We proved in `ProbabilityTheory.kernel.lintegral_compProd` that it
verifies `∫⁻ bc, f bc ∂((κ ⊗ₖ η) a) = ∫⁻ b, ∫⁻ c, f (c, b) ∂(η (b, a)) ∂(κ a)`. In this file, we
...
* `ProbabilityTheory.integral_compProd`: the integral against the composition-product is
  `∫ z, f z ∂((κ ⊗ₖ η) a) = ∫ x, ∫ y, f (y, x) ∂(η (x, a)) ∂(κ a)`
...
variable {α β γ E : Type*} {mα : MeasurableSpace α} {mβ : MeasurableSpace β}
  {mγ : MeasurableSpace γ} [NormedAddCommGroup E] {κ : kernel β α} [IsSFiniteKernel κ]
  {η : kernel γ (β × α)} [IsSFiniteKernel η] {a : α}

namespace ProbabilityTheory

theorem hasFiniteIntegral_prod_mk_left (a : α) {s : Set (γ × β)} (h2s : (κ ⊗ₖ η) a s ≠ ∞) :
    HasFiniteIntegral (fun b => (η (b, a) {c | (c, b) ∈ s}).toReal) (κ a) := by
  let t := toMeasurable ((κ ⊗ₖ η) a) s
  simp_rw [HasFiniteIntegral, ennnorm_eq_ofReal toReal_nonneg]
  calc
    ∫⁻ b, ENNReal.ofReal (η (b, a) {c | (c, b) ∈ s}).toReal ∂κ a
    _ ≤ ∫⁻ b, η (b, a) {c | (c, b) ∈ t} ∂κ a := by
...
theorem integrable_kernel_prod_mk_left (a : α) {s : Set (γ × β)} (hs : MeasurableSet s)
    (h2s : (κ ⊗ₖ η) a s ≠ ∞) : Integrable (fun b => (η (b, a) {c | (c, b) ∈ s}).toReal) (κ a) := by
  constructor
...
theorem _root_.MeasureTheory.AEStronglyMeasurable.integral_kernel_compProd [NormedSpace ℝ E]
    [CompleteSpace E] ⦃f : γ × β → E⦄ (hf : AEStronglyMeasurable f ((κ ⊗ₖ η) a)) :
    AEStronglyMeasurable (fun x => ∫ y, f (y, x) ∂η (x, a)) (κ a) :=
  ⟨fun x => ∫ y, hf.mk f (y, x) ∂η (x, a), hf.stronglyMeasurable_mk.integral_kernel_prod_right'', by
...
theorem _root_.MeasureTheory.AEStronglyMeasurable.compProd_mk_left {δ : Type*} [TopologicalSpace δ]
    {f : γ × β → δ} (hf : AEStronglyMeasurable f ((κ ⊗ₖ η) a)) :
    ∀ᵐ x ∂κ a, AEStronglyMeasurable (fun y => f (y, x)) (η (x, a)) := by
  filter_upwards [ae_ae_of_ae_compProd hf.ae_eq_mk] with x hx using
    ⟨fun y => hf.mk f (y, x), hf.stronglyMeasurable_mk.comp_measurable measurable_prod_mk_right, hx⟩
#align measure_theory.ae_strongly_measurable.comp_prod_mk_left MeasureTheory.AEStronglyMeasurable.compProd_mk_left

/-! ### Integrability -/


theorem hasFiniteIntegral_compProd_iff ⦃f : γ × β → E⦄ (h1f : StronglyMeasurable f) :
    HasFiniteIntegral f ((κ ⊗ₖ η) a) ↔
      (∀ᵐ x ∂κ a, HasFiniteIntegral (fun y => f (y, x)) (η (x, a))) ∧
        HasFiniteIntegral (fun x => ∫ y, ‖f (y, x)‖ ∂η (x, a)) (κ a) := by
  simp only [HasFiniteIntegral]
  rw [kernel.lintegral_compProd _ _ _ h1f.ennnorm]
  have : ∀ x, ∀ᵐ y ∂η (x, a), 0 ≤ ‖f (y, x)‖ := fun x => eventually_of_forall fun y => norm_nonneg _
  simp_rw [integral_eq_lintegral_of_nonneg_ae (this _)
      (h1f.norm.comp_measurable measurable_prod_mk_right).aestronglyMeasurable,
...
theorem hasFiniteIntegral_compProd_iff' ⦃f : γ × β → E⦄
...
theorem integrable_compProd_iff ⦃f : γ × β → E⦄ (hf : AEStronglyMeasurable f ((κ ⊗ₖ η) a)) :
    Integrable f ((κ ⊗ₖ η) a) ↔
      (∀ᵐ x ∂κ a, Integrable (fun y => f (y, x)) (η (x, a))) ∧
        Integrable (fun x => ∫ y, ‖f (y, x)‖ ∂η (x, a)) (κ a) := by
...
theorem _root_.MeasureTheory.Integrable.compProd_mk_left_ae ⦃f : γ × β → E⦄
    (hf : Integrable f ((κ ⊗ₖ η) a)) : ∀ᵐ x ∂κ a, Integrable (fun y => f (y, x)) (η (x, a)) :=
...
theorem _root_.MeasureTheory.Integrable.integral_norm_compProd ⦃f : γ × β → E⦄
    (hf : Integrable f ((κ ⊗ₖ η) a)) : Integrable (fun x => ∫ y, ‖f (y, x)‖ ∂η (x, a)) (κ a) :=
...
theorem _root_.MeasureTheory.Integrable.integral_compProd [NormedSpace ℝ E] [CompleteSpace E]
    ⦃f : γ × β → E⦄ (hf : Integrable f ((κ ⊗ₖ η) a)) :
    Integrable (fun x => ∫ y, f (y, x) ∂η (x, a)) (κ a) :=
  Integrable.mono hf.integral_norm_compProd hf.aestronglyMeasurable.integral_kernel_compProd <|
    eventually_of_forall fun x =>
      (norm_integral_le_integral_norm _).trans_eq <|
        (norm_of_nonneg <|
            integral_nonneg_of_ae <|
              eventually_of_forall fun y => (norm_nonneg (f (y, x)) : _)).symm
...
theorem kernel.integral_fn_integral_add ⦃f g : γ × β → E⦄ (F : E → E')
    (hf : Integrable f ((κ ⊗ₖ η) a)) (hg : Integrable g ((κ ⊗ₖ η) a)) :
    ∫ x, F (∫ y, f (y, x) + g (y, x) ∂η (x, a)) ∂κ a =
      ∫ x, F (∫ y, f (y, x) ∂η (x, a) + ∫ y, g (y, x) ∂η (x, a)) ∂κ a := by
...
theorem kernel.integral_fn_integral_sub ⦃f g : γ × β → E⦄ (F : E → E')
    (hf : Integrable f ((κ ⊗ₖ η) a)) (hg : Integrable g ((κ ⊗ₖ η) a)) :
    ∫ x, F (∫ y, f (y, x) - g (y, x) ∂η (x, a)) ∂κ a =
      ∫ x, F (∫ y, f (y, x) ∂η (x, a) - ∫ y, g (y, x) ∂η (x, a)) ∂κ a := by
  refine' integral_congr_ae _
...
theorem kernel.lintegral_fn_integral_sub ⦃f g : γ × β → E⦄ (F : E → ℝ≥0∞)
    (hf : Integrable f ((κ ⊗ₖ η) a)) (hg : Integrable g ((κ ⊗ₖ η) a)) :
    ∫⁻ x, F (∫ y, f (y, x) - g (y, x) ∂η (x, a)) ∂κ a =
      ∫⁻ x, F (∫ y, f (y, x) ∂η (x, a) - ∫ y, g (y, x) ∂η (x, a)) ∂κ a := by
...
theorem kernel.integral_integral_add ⦃f g : γ × β → E⦄ (hf : Integrable f ((κ ⊗ₖ η) a))
    (hg : Integrable g ((κ ⊗ₖ η) a)) :
    ∫ x, ∫ y, f (y, x) + g (y, x) ∂η (x, a) ∂κ a =
      ∫ x, ∫ y, f (y, x) ∂η (x, a) ∂κ a + ∫ x, ∫ y, g (y, x) ∂η (x, a) ∂κ a :=
...
theorem kernel.integral_integral_add' ⦃f g : γ × β → E⦄ (hf : Integrable f ((κ ⊗ₖ η) a))
    (hg : Integrable g ((κ ⊗ₖ η) a)) :
    ∫ x, ∫ y, (f + g) (y, x) ∂η (x, a) ∂κ a =
      ∫ x, ∫ y, f (y, x) ∂η (x, a) ∂κ a + ∫ x, ∫ y, g (y, x) ∂η (x, a) ∂κ a :=
  kernel.integral_integral_add hf hg
#align probability_theory.kernel.integral_integral_add' ProbabilityTheory.kernel.integral_integral_add'

theorem kernel.integral_integral_sub ⦃f g : γ × β → E⦄ (hf : Integrable f ((κ ⊗ₖ η) a))
    (hg : Integrable g ((κ ⊗ₖ η) a)) :
    ∫ x, ∫ y, f (y, x) - g (y, x) ∂η (x, a) ∂κ a =
      ∫ x, ∫ y, f (y, x) ∂η (x, a) ∂κ a - ∫ x, ∫ y, g (y, x) ∂η (x, a) ∂κ a :=
...
theorem kernel.continuous_integral_integral :
    -- Continuous fun f : β × α →₁[(κ ⊗ₖ η) a] E => ∫ x, ∫ y, f (y, x) ∂η (x, a) ∂κ a := by
    Continuous fun f : (MeasureTheory.Lp (α := γ × β) E 1 (((κ ⊗ₖ η) a) : Measure (γ × β))) =>
        ∫ x, ∫ y, f (y, x) ∂η (x, a) ∂κ a := by
  ...
  · exact fun i => ∫⁻ x, ∫⁻ y, ‖i (y, x) - g (y, x)‖₊ ∂η (x, a) ∂κ a
  swap; · exact fun i => lintegral_mono fun x => ennnorm_integral_le_lintegral_ennnorm _
  show
    Tendsto
      (fun i : γ × β →₁[(κ ⊗ₖ η) a] E => ∫⁻ x, ∫⁻ y : γ, ‖i (y, x) - g (y, x)‖₊ ∂η (x, a) ∂κ a)
      (𝓝 g) (𝓝 0)
  have : ∀ i : (MeasureTheory.Lp (α := γ × β) E 1 (((κ ⊗ₖ η) a) : Measure (γ × β))),
...
theorem integral_compProd :
    ∀ {f : γ × β → E} (_ : Integrable f ((κ ⊗ₖ η) a)),
      ∫ z, f z ∂(κ ⊗ₖ η) a = ∫ x, ∫ y, f (y, x) ∂η (x, a) ∂κ a := by
  apply Integrable.induction
  · intro c s hs h2s
    simp_rw [integral_indicator hs, ← indicator_comp_right, Function.comp]
    sorry
    --simp_rw [integral_indicator (measurable_prod_mk_left hs)]
    --simp_rw [MeasureTheory.set_integral_const,
    --  integral_smul_const]
    --congr 1
    --rw [integral_toReal]
    --rotate_left
...
theorem set_integral_compProd {f : γ × β → E} {s : Set β} {t : Set γ} (hs : MeasurableSet s)
    (ht : MeasurableSet t) (hf : IntegrableOn f (t ×ˢ s) ((κ ⊗ₖ η) a)) :
    ∫ z in t ×ˢ s, f z ∂(κ ⊗ₖ η) a = ∫ x in s, ∫ y in t, f (y, x) ∂η (x, a) ∂κ a := by
  -- Porting note: `compProd_restrict` needed some explicit argumnts
  rw [← kernel.restrict_apply (κ ⊗ₖ η) (ht.prod hs), ← compProd_restrict hs ht, integral_compProd]
...
theorem set_integral_compProd_univ_right (f : γ × β → E) {s : Set β} (hs : MeasurableSet s)
    (hf : IntegrableOn f (univ ×ˢ s) ((κ ⊗ₖ η) a)) :
    ∫ z in univ ×ˢ s, f z ∂(κ ⊗ₖ η) a = ∫ x in s, ∫ y, f (y, x) ∂η (x, a) ∂κ a := by
...
theorem set_integral_compProd_univ_left (f : γ × β → E) {t : Set γ} (ht : MeasurableSet t)
    (hf : IntegrableOn f (t ×ˢ univ) ((κ ⊗ₖ η) a)) :
    ∫ z in t ×ˢ univ, f z ∂(κ ⊗ₖ η) a = ∫ x, ∫ y in t, f (y, x) ∂η (x, a) ∂κ a := by

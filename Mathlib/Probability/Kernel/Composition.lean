...
We define
* the composition-product `κ ⊗ₖ η` of two s-finite kernels `κ : kernel β α` and
  `η : kernel γ (β × α)`, a kernel from `α` to `γ × β`.
* the map and comap of a kernel along a measurable function.
* the composition `η ∘ₖ κ` of kernels `κ : kernel β α` and `η : kernel γ β`, kernel from `α` to
  `γ`.
* the product `κ ×ₖ η` of s-finite kernels `κ : kernel β α` and `η : kernel γ α`,
  a kernel from `α` to `γ × β`.

A note on names:
The composition-product `kernel β α → kernel γ (β × α) → kernel (γ × β) α` is named composition in
...
* `compProd (κ : kernel β α) (η : kernel γ (β × α)) : kernel (γ × β) α`: composition-product of 2
  s-finite kernels. We define a notation `κ ⊗ₖ η = compProd κ η`.
  `∫⁻ bc, f bc ∂((κ ⊗ₖ η) a) = ∫⁻ b, ∫⁻ c, f (c, b) ∂(η (b, a)) ∂(κ a)`
* `map (κ : kernel β α) (f : β → γ) (hf : Measurable f) : kernel γ α`
  `∫⁻ c, g c ∂(map κ f hf a) = ∫⁻ b, g (f b) ∂(κ a)`
* `comap (κ : kernel β α) (f : γ → α) (hf : Measurable f) : kernel γ β`
  `∫⁻ b, g b ∂(comap κ f hf c) = ∫⁻ b, g b ∂(κ (f c))`
* `comp (η : kernel γ β) (κ : kernel β α) : kernel γ α`: composition of 2 kernels.
  We define a notation `η ∘ₖ κ = comp η κ`.
  `∫⁻ c, g c ∂((η ∘ₖ κ) a) = ∫⁻ b, ∫⁻ c, g c ∂(η b) ∂(κ a)`
* `prod (κ : kernel β α) (η : kernel γ α) : kernel (γ × β) α`: product of 2 s-finite kernels.
  `∫⁻ bc, f bc ∂((κ ×ₖ η) a) = ∫⁻ b, ∫⁻ c, f (c, b) ∂(η a) ∂(κ a)`
...
`compProd : kernel β α → kernel γ (β × α) → kernel (γ × β) α`.
-/


variable {γ : Type*} {mγ : MeasurableSpace γ} {s : Set (γ × β)}
...
noncomputable def compProdFun (κ : kernel β α) (η : kernel γ (β × α)) (a : α) (s : Set (γ × β)) :
    ℝ≥0∞ :=
  ∫⁻ b, η (b, a) {c | (c, b) ∈ s} ∂κ a
#align probability_theory.kernel.comp_prod_fun ProbabilityTheory.kernel.compProdFun

theorem compProdFun_empty (κ : kernel β α) (η : kernel γ (β × α)) (a : α) :
...
theorem compProdFun_iUnion (κ : kernel β α) (η : kernel γ (β × α)) [IsSFiniteKernel η] (a : α)
    (f : ℕ → Set (γ × β)) (hf_meas : ∀ i, MeasurableSet (f i))
    (hf_disj : Pairwise (Disjoint on f)) :
    compProdFun κ η a (⋃ i, f i) = ∑' i, compProdFun κ η a (f i) := by
  have h_Union :
    (fun b => η (b, a) {c : γ | (c, b) ∈ ⋃ i, f i}) = fun b =>
      η (b, a) (⋃ i, {c : γ | (c, b) ∈ f i}) := by
    ext1 b
    congr with c
    simp only [Set.mem_iUnion, Set.iSup_eq_iUnion, Set.mem_setOf_eq]
  rw [compProdFun, h_Union]
  have h_tsum :
    (fun b => η (b, a) (⋃ i, {c : γ | (c, b) ∈ f i})) = fun b =>
      ∑' i, η (b, a) {c : γ | (c, b) ∈ f i} := by
    ext1 b
    rw [measure_iUnion]
    · intro i j hij s hsi hsj c hcs
      have hbci : {(c, b)} ⊆ f i := by rw [Set.singleton_subset_iff]; exact hsi hcs
      have hbcj : {(c, b)} ⊆ f j := by rw [Set.singleton_subset_iff]; exact hsj hcs
    ...
    · -- Porting note: behavior of `@` changed relative to lean 3, was
      ...
      exact fun i => (@measurable_prod_mk_right γ β _ _ b) (hf_meas i)
  rw [h_tsum, lintegral_tsum]
  · rfl
  · intro i
    have hm : MeasurableSet {p : γ × β × α | (p.1, p.2.1) ∈ f i} :=
      measurable_fst.prod_mk measurable_snd.fst (hf_meas i)
    exact ((measurable_kernel_prod_mk_left hm).comp
      (measurable_prod_mk_right : Measurable (fun b : β ↦ (b, a)))).aemeasurable
#align probability_theory.kernel.comp_prod_fun_Union ProbabilityTheory.kernel.compProdFun_iUnion

theorem compProdFun_tsum_right (κ : kernel β α) (η : kernel γ (β × α)) [IsSFiniteKernel η] (a : α)
    (hs : MeasurableSet s) : compProdFun κ η a s = ∑' n, compProdFun κ (seq η n) a s := by
  simp_rw [compProdFun, (measure_sum_seq η _).symm]
  have :
    ∫⁻ b, Measure.sum (fun n => seq η n (b, a)) {c : γ | (c, b) ∈ s} ∂κ a =
      ∫⁻ b, ∑' n, seq η n (b, a) {c : γ | (c, b) ∈ s} ∂κ a := by
    congr
    ext1 b
    rw [Measure.sum_apply]
    exact measurable_prod_mk_right hs
  rw [this, lintegral_tsum]
  exact fun n => ((measurable_kernel_prod_mk_left (κ := (seq η n))
    ((measurable_fst.prod_mk measurable_snd.fst) hs)).comp measurable_prod_mk_right).aemeasurable
#align probability_theory.kernel.comp_prod_fun_tsum_right ProbabilityTheory.kernel.compProdFun_tsum_right

theorem compProdFun_tsum_left (κ : kernel β α) (η : kernel γ (β × α)) [IsSFiniteKernel κ] (a : α)
    (s : Set (γ × β)) : compProdFun κ η a s = ∑' n, compProdFun (seq κ n) η a s := by
...
theorem compProdFun_eq_tsum (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
...
theorem measurable_compProdFun_of_finite (κ : kernel β α) [IsFiniteKernel κ] (η : kernel γ (β × α))
    [IsFiniteKernel η] (hs : MeasurableSet s) : Measurable fun a => compProdFun κ η a s := by
  simp only [compProdFun]
  have h_meas : Measurable (Function.uncurry fun b a => η (b, a) {c : γ | (c, b) ∈ s}) := by
    have :
      (Function.uncurry fun b a => η (b, a) {c : γ | (c, b) ∈ s}) = fun p =>
        η p {c : γ | (c, p.1) ∈ s} := by
      ext1 p
      rw [Function.uncurry_apply_pair]
    rw [this]
    refine measurable_kernel_prod_mk_left (measurable_fst.prod_mk measurable_snd.fst hs)
...
theorem measurable_compProdFun (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
  ...
  simp only [compProdFun]
  have h_meas : Measurable (Function.uncurry fun b a => seq η n (b, a) {c : γ | (c, b) ∈ s}) := by
    have :
      (Function.uncurry fun b a => seq η n (b, a) {c : γ | (c, b) ∈ s}) = fun p =>
        seq η n p {c : γ | (c, p.1) ∈ s} := by
      ext1 p
      rw [Function.uncurry_apply_pair]
    rw [this]
    exact measurable_kernel_prod_mk_left (measurable_fst.prod_mk measurable_snd.fst hs)
  exact h_meas.lintegral_kernel_prod_right
...
`∫⁻ bc, f bc ∂(compProd κ η a) = ∫⁻ b, ∫⁻ c, f (c, b) ∂(η (b, a)) ∂(κ a)`
...
noncomputable def compProd (κ : kernel β α) (η : kernel γ (β × α)) : kernel (γ × β) α :=
...
theorem compProd_apply_eq_compProdFun (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) (hs : MeasurableSet s) :
...
theorem compProd_of_not_isSFiniteKernel_left (κ : kernel β α) (η : kernel γ (β × α))
    (h : ¬ IsSFiniteKernel κ) :
    κ ⊗ₖ η = 0 := by
  rw [compProd, dif_neg]
  simp [h]

theorem compProd_of_not_isSFiniteKernel_right (κ : kernel β α) (η : kernel γ (β × α))
    (h : ¬ IsSFiniteKernel η) :
    κ ⊗ₖ η = 0 := by
  rw [compProd, dif_neg]
  simp [h]

theorem compProd_apply (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) (hs : MeasurableSet s) :
    (κ ⊗ₖ η) a s = ∫⁻ b, η (b, a) {c | (c, b) ∈ s} ∂κ a :=
...
theorem le_compProd_apply (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) (s : Set (γ × β)) :
    ∫⁻ b, η (b, a) {c | (c, b) ∈ s} ∂κ a ≤ (κ ⊗ₖ η) a s :=
  calc
    ∫⁻ b, η (b, a) {c | (c, b) ∈ s} ∂κ a ≤
        ∫⁻ b, η (b, a) {c | (c, b) ∈ toMeasurable ((κ ⊗ₖ η) a) s} ∂κ a :=
...
@[simp]
lemma compProd_zero_left (κ : kernel γ (β × α)) :
    (0 : kernel β α) ⊗ₖ κ = 0 := by
...
@[simp]
lemma compProd_zero_right (κ : kernel β α) (γ : Type*) [MeasurableSpace γ] :
    κ ⊗ₖ (0 : kernel γ (β × α)) = 0 := by
  by_cases h : IsSFiniteKernel κ
  · ext a s hs
    rw [kernel.compProd_apply _ _ _ hs]
    simp
...
variable {κ : kernel β α} [IsSFiniteKernel κ] {η : kernel γ (β × α)} [IsSFiniteKernel η] {a : α}

theorem ae_kernel_lt_top (a : α) (h2s : (κ ⊗ₖ η) a s ≠ ∞) :
    ∀ᵐ b ∂κ a, η (b, a) {c | (c, b) ∈ s} < ∞ := by
  let t := toMeasurable ((κ ⊗ₖ η) a) s
  have : ∀ b : β, η (b, a) {c | (c, b) ∈ s} ≤ η (b, a) {c | (c, b) ∈ t} := fun b =>
  ...
  have ht_lt_top : ∀ᵐ b ∂κ a, η (b, a) {c | (c, b) ∈ t} < ∞ := by
...
theorem compProd_null (a : α) (hs : MeasurableSet s) :
    (κ ⊗ₖ η) a s = 0 ↔ (fun b => η (b, a) {c | (c, b) ∈ s}) =ᵐ[κ a] 0 := by
...
theorem ae_null_of_compProd_null (h : (κ ⊗ₖ η) a s = 0) :
    (fun b => η (b, a) {c | (c, b) ∈ s}) =ᵐ[κ a] 0 := by
...
theorem ae_ae_of_ae_compProd {p : γ × β → Prop} (h : ∀ᵐ bc ∂(κ ⊗ₖ η) a, p bc) :
    ∀ᵐ b ∂κ a, ∀ᵐ c ∂η (b, a), p (c, b) :=
...
theorem compProd_restrict {s : Set β} {t : Set γ} (hs : MeasurableSet s) (ht : MeasurableSet t) :
    kernel.restrict κ hs ⊗ₖ kernel.restrict η ht = kernel.restrict (κ ⊗ₖ η) (ht.prod hs) := by
  ext a u hu
  rw [compProd_apply _ _ _ hu, restrict_apply' _ _ _ hu,
    compProd_apply _ _ _ (hu.inter (ht.prod hs))]
  simp only [kernel.restrict_apply, Measure.restrict_apply' ht, Set.mem_inter_iff,
    Set.prod_mk_mem_set_prod_eq]
  have :
    ∀ b, η (b, a) {c : γ | (c, b) ∈ u ∧ c ∈ t ∧ b ∈ s} =
        s.indicator (fun b => η (b, a) ({c : γ | (c, b) ∈ u} ∩ t)) b := by
    intro b
    classical
...
theorem compProd_restrict_left {s : Set β} (hs : MeasurableSet s) :
    kernel.restrict κ hs ⊗ₖ η = kernel.restrict (κ ⊗ₖ η) (MeasurableSet.univ.prod hs) := by
...
theorem compProd_restrict_right {t : Set γ} (ht : MeasurableSet t) :
    κ ⊗ₖ kernel.restrict η ht = kernel.restrict (κ ⊗ₖ η) (ht.prod MeasurableSet.univ) := by
  rw [← compProd_restrict]; congr; exact kernel.restrict_univ.symm
...
theorem lintegral_compProd' (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) {f : γ → β → ℝ≥0∞} (hf : Measurable (Function.uncurry f)) :
    ∫⁻ bc, f bc.1 bc.2 ∂(κ ⊗ₖ η) a = ∫⁻ b, ∫⁻ c, f c b ∂η (b, a) ∂κ a := by
  let F : ℕ → SimpleFunc (γ × β) ℝ≥0∞ := SimpleFunc.eapprox (Function.uncurry f)
  ...
  have : ∀ b, ∫⁻ c, ⨆ n, F n (c, b) ∂η (b, a) = ⨆ n, ∫⁻ c, F n (c, b) ∂η (b, a) := by
    intro a
    rw [lintegral_iSup]
  ...
  have h_some_meas_integral :
    ∀ f' : SimpleFunc (γ × β) ℝ≥0∞, Measurable fun b => ∫⁻ c, f' (c, b) ∂η (b, a) := by
    intro f'
    have :
      (fun b => ∫⁻ c, f' (c, b) ∂η (b, a)) =
        (fun ab => ∫⁻ c, f' (c, ab.1) ∂η ab) ∘ fun b => (b, a) :=
      by ext1 ab; rfl
    rw [this]
    refine' Measurable.comp _ measurable_prod_mk_right
    exact Measurable.lintegral_kernel_prod_right
        ((SimpleFunc.measurable _).comp (measurable_fst.prod_mk measurable_snd.fst))
  rw [lintegral_iSup]
  rotate_left
  ...
  refine' SimpleFunc.induction (P := fun f => (∫⁻ (a : γ × β), f a ∂(κ ⊗ₖ η) a =
      ∫⁻ (a_1 : β), ∫⁻ (c : γ), f (c, a_1) ∂η (a_1, a) ∂κ a)) _ _ (F n)
  · intro c s hs
    ...
    swap
    · exact (measurable_kernel_prod_mk_left ((measurable_fst.prod_mk measurable_snd.fst) hs)).comp
        measurable_prod_mk_right
    congr
    ext1 b
    rw [lintegral_indicator_const_comp measurable_prod_mk_right hs]
    rfl
  · intro f f' _ hf_eq hf'_eq
    simp_rw [SimpleFunc.coe_add, Pi.add_apply]
    change
      ∫⁻ x, (f : γ × β → ℝ≥0∞) x + f' x ∂(κ ⊗ₖ η) a =
        ∫⁻ b, ∫⁻ c : γ, f (c, b) + f' (c, b) ∂η (b, a) ∂κ a
    rw [lintegral_add_left (SimpleFunc.measurable _), hf_eq, hf'_eq, ← lintegral_add_left]
    swap
    · exact h_some_meas_integral f
    congr with b
    rw [lintegral_add_left]
    exact (SimpleFunc.measurable _).comp measurable_prod_mk_right
#align probability_theory.kernel.lintegral_comp_prod' ProbabilityTheory.kernel.lintegral_compProd'

/-- Lebesgue integral against the composition-product of two kernels. -/
theorem lintegral_compProd (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) {f : γ × β → ℝ≥0∞} (hf : Measurable f) :
    ∫⁻ bc, f bc ∂(κ ⊗ₖ η) a = ∫⁻ b, ∫⁻ c, f (c, b) ∂η (b, a) ∂κ a := by
  let g := Function.curry f
  change ∫⁻ bc, f bc ∂(κ ⊗ₖ η) a = ∫⁻ b, ∫⁻ c, g c b ∂η (b, a) ∂κ a
...
/-- Lebesgue integral against the composition-product of two kernels. -/
theorem lintegral_compProd₀ (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) {f : γ × β → ℝ≥0∞} (hf : AEMeasurable f ((κ ⊗ₖ η) a)) :
    ∫⁻ z, f z ∂(κ ⊗ₖ η) a = ∫⁻ x, ∫⁻ y, f (y, x) ∂η (x, a) ∂κ a := by
  have A : ∫⁻ z, f z ∂(κ ⊗ₖ η) a = ∫⁻ z, hf.mk f z ∂(κ ⊗ₖ η) a := lintegral_congr_ae hf.ae_eq_mk
  have B : ∫⁻ x, ∫⁻ y, f (y, x) ∂η (x, a) ∂κ a = ∫⁻ x, ∫⁻ y, hf.mk f (y, x) ∂η (x, a) ∂κ a := by
...
theorem set_lintegral_compProd (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) {f : γ × β → ℝ≥0∞} (hf : Measurable f) {s : Set β} {t : Set γ}
    (hs : MeasurableSet s) (ht : MeasurableSet t) :
    ∫⁻ z in t ×ˢ s, f z ∂(κ ⊗ₖ η) a = ∫⁻ x in s, ∫⁻ y in t, f (y, x) ∂η (x, a) ∂κ a := by
  simp_rw [← kernel.restrict_apply (κ ⊗ₖ η) (ht.prod hs), ← compProd_restrict hs ht,
...
theorem set_lintegral_compProd_univ_right (κ : kernel β α) [IsSFiniteKernel κ]
    (η : kernel γ (β × α)) [IsSFiniteKernel η] (a : α) {f : γ × β → ℝ≥0∞} (hf : Measurable f)
    {s : Set β} (hs : MeasurableSet s) :
    ∫⁻ z in Set.univ ×ˢ s, f z ∂(κ ⊗ₖ η) a = ∫⁻ x in s, ∫⁻ y, f (y, x) ∂η (x, a) ∂κ a := by
...
theorem set_lintegral_compProd_univ_left (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) {f : γ × β → ℝ≥0∞} (hf : Measurable f) {t : Set γ}
    (ht : MeasurableSet t) :
    ∫⁻ z in t ×ˢ Set.univ, f z ∂(κ ⊗ₖ η) a = ∫⁻ x, ∫⁻ y in t, f (y, x) ∂η (x, a) ∂κ a := by
...
theorem compProd_eq_tsum_compProd (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
    [IsSFiniteKernel η] (a : α) (hs : MeasurableSet s) :
...
theorem compProd_eq_sum_compProd (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α))
...
theorem compProd_eq_sum_compProd_left (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ (β × α)) :
...
theorem compProd_eq_sum_compProd_right (κ : kernel β α) (η : kernel γ (β × α))
...
instance IsMarkovKernel.compProd (κ : kernel β α) [IsMarkovKernel κ] (η : kernel γ (β × α))
...
theorem compProd_apply_univ_le (κ : kernel β α) (η : kernel γ (β × α)) [IsFiniteKernel η] (a : α) :
  ...
  calc
    ∫⁻ b, η (b, a) Set.univ ∂κ a ≤ ∫⁻ _, Cη ∂κ a :=
      lintegral_mono fun b => measure_le_bound η (b, a) Set.univ
...
instance IsFiniteKernel.compProd (κ : kernel β α) [IsFiniteKernel κ] (η : kernel γ (β × α))
...
instance IsSFiniteKernel.compProd (κ : kernel β α) (η : kernel γ (β × α)) :
...
noncomputable def map (κ : kernel β α) (f : β → γ) (hf : Measurable f) : kernel γ α where
  val a := (κ a).map f
...
theorem map_apply (κ : kernel β α) (hf : Measurable f) (a : α) : map κ f hf a = (κ a).map f :=
  rfl
#align probability_theory.kernel.map_apply ProbabilityTheory.kernel.map_apply

theorem map_apply' (κ : kernel β α) (hf : Measurable f) (a : α) {s : Set γ} (hs : MeasurableSet s) :
    map κ f hf a s = κ a (f ⁻¹' s) := by rw [map_apply, Measure.map_apply hf hs]
#align probability_theory.kernel.map_apply' ProbabilityTheory.kernel.map_apply'

@[simp]
lemma map_zero (hf : Measurable f) : kernel.map (0 : kernel β α) f hf = 0 := by
  ext; rw [kernel.map_apply]; simp

nonrec theorem lintegral_map (κ : kernel β α) (hf : Measurable f) (a : α) {g' : γ → ℝ≥0∞}
...
theorem sum_map_seq (κ : kernel β α) [IsSFiniteKernel κ] (hf : Measurable f) :
...
instance IsMarkovKernel.map (κ : kernel β α) [IsMarkovKernel κ] (hf : Measurable f) :
...
instance IsFiniteKernel.map (κ : kernel β α) [IsFiniteKernel κ] (hf : Measurable f) :
...
instance IsSFiniteKernel.map (κ : kernel β α) [IsSFiniteKernel κ] (hf : Measurable f) :
...
is again a Markov kernel. -/
def comap (κ : kernel β α) (g : γ → α) (hg : Measurable g) : kernel β γ where
  val a := κ (g a)
  property := (kernel.measurable κ).comp hg
#align probability_theory.kernel.comap ProbabilityTheory.kernel.comap

theorem comap_apply (κ : kernel β α) (hg : Measurable g) (c : γ) : comap κ g hg c = κ (g c) :=
  rfl
#align probability_theory.kernel.comap_apply ProbabilityTheory.kernel.comap_apply

theorem comap_apply' (κ : kernel β α) (hg : Measurable g) (c : γ) (s : Set β) :
    comap κ g hg c s = κ (g c) s :=
  rfl
#align probability_theory.kernel.comap_apply' ProbabilityTheory.kernel.comap_apply'

@[simp]
lemma comap_zero (hg : Measurable g) : kernel.comap (0 : kernel β α) g hg = 0 := by
  ext; rw [kernel.comap_apply]; simp

theorem lintegral_comap (κ : kernel β α) (hg : Measurable g) (c : γ) (g' : β → ℝ≥0∞) :
    ∫⁻ b, g' b ∂comap κ g hg c = ∫⁻ b, g' b ∂κ (g c) :=
  rfl
#align probability_theory.kernel.lintegral_comap ProbabilityTheory.kernel.lintegral_comap

theorem sum_comap_seq (κ : kernel β α) [IsSFiniteKernel κ] (hg : Measurable g) :
...
instance IsMarkovKernel.comap (κ : kernel β α) [IsMarkovKernel κ] (hg : Measurable g) :
...
instance IsFiniteKernel.comap (κ : kernel β α) [IsFiniteKernel κ] (hg : Measurable g) :
...
instance IsSFiniteKernel.comap (κ : kernel β α) [IsSFiniteKernel κ] (hg : Measurable g) :
...
/-- Define a `kernel (γ × α) β` from a `kernel β α` by taking the comap of the projection. -/
def prodMkLeft (γ : Type*) [MeasurableSpace γ] (κ : kernel β α) : kernel β (γ × α) :=
...
@[simp]
theorem prodMkLeft_apply (κ : kernel β α) (ca : γ × α) : prodMkLeft γ κ ca = κ ca.snd :=
  rfl
#align probability_theory.kernel.prod_mk_left_apply ProbabilityTheory.kernel.prodMkLeft_apply

theorem prodMkLeft_apply' (κ : kernel β α) (ca : γ × α) (s : Set β) :
...
theorem lintegral_prodMkLeft (κ : kernel β α) (ca : γ × α) (g : β → ℝ≥0∞) :
...
instance IsMarkovKernel.prodMkLeft (κ : kernel β α) [IsMarkovKernel κ] :
...
instance IsFiniteKernel.prodMkLeft (κ : kernel β α) [IsFiniteKernel κ] :
...
instance IsSFiniteKernel.prodMkLeft (κ : kernel β α) [IsSFiniteKernel κ] :
...
/-- Define a `kernel (γ × α) β` from a `kernel β α` by taking the comap of the projection. -/
def prodMkRight (κ : kernel β α) (γ : Type*) [MeasurableSpace γ] : kernel β (α × γ) :=
  comap κ Prod.fst measurable_fst

variable {γ : Type*} {mγ : MeasurableSpace γ} {f : β → γ} {g : γ → α}

@[simp]
theorem prodMkRight_apply (κ : kernel β α) (ca : α × γ) : prodMkRight κ γ ca = κ ca.fst := rfl

theorem prodMkRight_apply' (κ : kernel β α) (ca : α × γ) (s : Set β) :
    prodMkRight κ γ ca s = κ ca.fst s := rfl

@[simp]
lemma prodMkRight_zero : kernel.prodMkRight (0 : kernel β γ) α = 0 := by
  ext x s _; simp

theorem lintegral_prodMkRight (κ : kernel β α) (ca : α × γ) (g : β → ℝ≥0∞) :
    ∫⁻ b, g b ∂prodMkRight κ γ ca = ∫⁻ b, g b ∂κ ca.fst :=
  rfl

instance IsMarkovKernel.prodMkRight (κ : kernel β α) [IsMarkovKernel κ] :
    IsMarkovKernel (prodMkRight κ γ) := by rw [kernel.prodMkRight]; infer_instance

instance IsFiniteKernel.prodMkRight (κ : kernel β α) [IsFiniteKernel κ] :
    IsFiniteKernel (prodMkRight κ γ) := by rw [kernel.prodMkRight]; infer_instance

instance IsSFiniteKernel.prodMRight (κ : kernel β α) [IsSFiniteKernel κ] :
    IsSFiniteKernel (prodMkRight κ γ) := by rw [kernel.prodMkRight]; infer_instance

/-- Define a `kernel (β × α) γ` from a `kernel γ (β × α)` by taking the comap of `Prod.swap`. -/
def swapLeft (κ : kernel γ (β × α)) : kernel γ (α × β) :=
  comap κ Prod.swap measurable_swap
#align probability_theory.kernel.swap_left ProbabilityTheory.kernel.swapLeft

@[simp]
theorem swapLeft_apply (κ : kernel γ (β × α)) (a : α × β) : swapLeft κ a = κ a.swap := rfl
#align probability_theory.kernel.swap_left_apply ProbabilityTheory.kernel.swapLeft_apply

theorem swapLeft_apply' (κ : kernel γ (β × α)) (a : α × β) (s : Set γ) :
    swapLeft κ a s = κ a.swap s := rfl
#align probability_theory.kernel.swap_left_apply' ProbabilityTheory.kernel.swapLeft_apply'

theorem lintegral_swapLeft (κ : kernel γ (β × α)) (a : α × β) (g : γ → ℝ≥0∞) :
...
instance IsMarkovKernel.swapLeft (κ : kernel γ (β × α)) [IsMarkovKernel κ] :
...
instance IsFiniteKernel.swapLeft (κ : kernel γ (β × α)) [IsFiniteKernel κ] :
...
instance IsSFiniteKernel.swapLeft (κ : kernel γ (β × α)) [IsSFiniteKernel κ] :
...
@[simp]
lemma swapLeft_prodMkLeft (κ : kernel β α) (γ : Type*) [MeasurableSpace γ] :
    swapLeft (prodMkLeft γ κ) = prodMkRight κ γ := rfl

@[simp]
lemma swapLeft_prodMkRight (κ : kernel β α) (γ : Type*) [MeasurableSpace γ] :
    swapLeft (prodMkRight κ γ) = prodMkLeft γ κ := rfl

/-- Define a `kernel α (γ × β)` from a `kernel (γ × β) α` by taking the map of `Prod.swap`. -/
noncomputable def swapRight (κ : kernel (γ × β) α) : kernel (β × γ) α :=
  map κ Prod.swap measurable_swap
#align probability_theory.kernel.swap_right ProbabilityTheory.kernel.swapRight

theorem swapRight_apply (κ : kernel (γ × β) α) (a : α) : swapRight κ a = (κ a).map Prod.swap :=
  rfl
#align probability_theory.kernel.swap_right_apply ProbabilityTheory.kernel.swapRight_apply

theorem swapRight_apply' (κ : kernel (γ × β) α) (a : α) {s : Set (β × γ)} (hs : MeasurableSet s) :
    swapRight κ a s = κ a {p | p.swap ∈ s} := by
  rw [swapRight_apply, Measure.map_apply measurable_swap hs]; rfl
#align probability_theory.kernel.swap_right_apply' ProbabilityTheory.kernel.swapRight_apply'

theorem lintegral_swapRight (κ : kernel (γ × β) α) (a : α) {g : β × γ → ℝ≥0∞} (hg : Measurable g) :
    ∫⁻ c, g c ∂swapRight κ a = ∫⁻ bc : γ × β, g bc.swap ∂κ a := by
...
instance IsMarkovKernel.swapRight (κ : kernel (γ × β) α) [IsMarkovKernel κ] :
...
instance IsFiniteKernel.swapRight (κ : kernel (γ × β) α) [IsFiniteKernel κ] :
...
instance IsSFiniteKernel.swapRight (κ : kernel (γ × β) α) [IsSFiniteKernel κ] :
...
/-- Define a `kernel β α` from a `kernel (γ × β) α` by taking the map of the first projection. -/
noncomputable def fst (κ : kernel (γ × β) α) : kernel γ α :=
  map κ Prod.fst measurable_fst
#align probability_theory.kernel.fst ProbabilityTheory.kernel.fst

theorem fst_apply (κ : kernel (γ × β) α) (a : α) : fst κ a = (κ a).map Prod.fst :=
  rfl
#align probability_theory.kernel.fst_apply ProbabilityTheory.kernel.fst_apply

theorem fst_apply' (κ : kernel (γ × β) α) (a : α) {s : Set γ} (hs : MeasurableSet s) :
    fst κ a s = κ a {p | p.1 ∈ s} := by rw [fst_apply, Measure.map_apply measurable_fst hs]; rfl
#align probability_theory.kernel.fst_apply' ProbabilityTheory.kernel.fst_apply'

@[simp]
lemma fst_zero : fst (0 : kernel (γ × β) α) = 0 := by simp [fst]

theorem lintegral_fst (κ : kernel (γ × β) α) (a : α) {g : γ → ℝ≥0∞} (hg : Measurable g) :
    ∫⁻ c, g c ∂fst κ a = ∫⁻ bc : γ × β, g bc.fst ∂κ a := by
...
instance IsMarkovKernel.fst (κ : kernel (γ × β) α) [IsMarkovKernel κ] : IsMarkovKernel (fst κ) := by
...
instance IsFiniteKernel.fst (κ : kernel (γ × β) α) [IsFiniteKernel κ] : IsFiniteKernel (fst κ) := by
  rw [kernel.fst]; infer_instance
#align probability_theory.kernel.is_finite_kernel.fst ProbabilityTheory.kernel.IsFiniteKernel.fst

instance IsSFiniteKernel.fst (κ : kernel (γ × β) α) [IsSFiniteKernel κ] : IsSFiniteKernel (fst κ) :=
...
lemma fst_map_prod (κ : kernel β α) {f : β → γ} {g : β → δ}
...
/-- Define a `kernel γ α` from a `kernel (γ × β) α` by taking the map of the second projection. -/
noncomputable def snd (κ : kernel (γ × β) α) : kernel β α :=
  map κ Prod.snd measurable_snd
#align probability_theory.kernel.snd ProbabilityTheory.kernel.snd

theorem snd_apply (κ : kernel (γ × β) α) (a : α) : snd κ a = (κ a).map Prod.snd :=
  rfl
#align probability_theory.kernel.snd_apply ProbabilityTheory.kernel.snd_apply

theorem snd_apply' (κ : kernel (γ × β) α) (a : α) {s : Set β} (hs : MeasurableSet s) :
    snd κ a s = κ a {p | p.2 ∈ s} := by rw [snd_apply, Measure.map_apply measurable_snd hs]; rfl
#align probability_theory.kernel.snd_apply' ProbabilityTheory.kernel.snd_apply'

@[simp]
lemma snd_zero : snd (0 : kernel (γ × β) α) = 0 := by simp [snd]

theorem lintegral_snd (κ : kernel (γ × β) α) (a : α) {g : β → ℝ≥0∞} (hg : Measurable g) :
    ∫⁻ c, g c ∂snd κ a = ∫⁻ bc : γ × β, g bc.snd ∂κ a := by
...
instance IsMarkovKernel.snd (κ : kernel (γ × β) α) [IsMarkovKernel κ] : IsMarkovKernel (snd κ) := by
...
instance IsFiniteKernel.snd (κ : kernel (γ × β) α) [IsFiniteKernel κ] : IsFiniteKernel (snd κ) := by
  rw [kernel.snd]; infer_instance
#align probability_theory.kernel.is_finite_kernel.snd ProbabilityTheory.kernel.IsFiniteKernel.snd

instance IsSFiniteKernel.snd (κ : kernel (γ × β) α) [IsSFiniteKernel κ] : IsSFiniteKernel (snd κ) :=
...
lemma snd_map_prod (κ : kernel β α) {f : β → γ} {g : β → δ}
    (hf : Measurable f) (hg : Measurable g) :
...
@[simp]
lemma snd_compProd (κ : kernel β α) (η : kernel γ (β × α)) [IsSFiniteKernel κ] [IsMarkovKernel η] :
    snd (κ ⊗ₖ η) = κ := by
  ext x s hs
  rw [snd_apply' _ _ hs, compProd_apply]
  swap; · exact measurable_snd hs
  simp only [Set.mem_setOf_eq]
  classical
  have : ∀ b : β, η (b, x) {_c | b ∈ s} = s.indicator (fun _ ↦ 1) b := by
    intro b
...
@[simp]
lemma fst_swapRight (κ : kernel (γ × β) α) : fst (swapRight κ) = snd κ := by
  ext a s hs
...
@[simp]
lemma snd_swapRight (κ : kernel (γ × β) α) : snd (swapRight κ) = fst κ := by
  ext a s hs
...
/-- Composition of two kernels. -/
noncomputable def comp (η : kernel γ β) (κ : kernel β α) : kernel γ α where
  val a := (κ a).bind η
...
theorem comp_apply (η : kernel γ β) (κ : kernel β α) (a : α) : (η ∘ₖ κ) a = (κ a).bind η :=
  rfl
#align probability_theory.kernel.comp_apply ProbabilityTheory.kernel.comp_apply

theorem comp_apply' (η : kernel γ β) (κ : kernel β α) (a : α) {s : Set γ} (hs : MeasurableSet s) :
    (η ∘ₖ κ) a s = ∫⁻ b, η b s ∂κ a := by
  rw [comp_apply, Measure.bind_apply hs (kernel.measurable _)]
#align probability_theory.kernel.comp_apply' ProbabilityTheory.kernel.comp_apply'

theorem comp_eq_fst_compProd (η : kernel γ β) [IsSFiniteKernel η] (κ : kernel β α)
    [IsSFiniteKernel κ] : η ∘ₖ κ = fst (κ ⊗ₖ prodMkRight η α) := by
  ext a s hs
  rw [comp_apply' _ _ _ hs, fst_apply' _ _ hs, compProd_apply]
  swap
  · exact measurable_fst hs
  simp only [Set.mem_setOf_eq, Set.setOf_mem_eq, prodMkRight_apply' _ _ s]
#align probability_theory.kernel.comp_eq_snd_comp_prod ProbabilityTheory.kernel.comp_eq_fst_compProd

theorem lintegral_comp (η : kernel γ β) (κ : kernel β α) (a : α) {g : γ → ℝ≥0∞}
...
instance IsMarkovKernel.comp (η : kernel γ β) [IsMarkovKernel η] (κ : kernel β α)
    [IsMarkovKernel κ] : IsMarkovKernel (η ∘ₖ κ) := by rw [comp_eq_fst_compProd]; infer_instance
#align probability_theory.kernel.is_markov_kernel.comp ProbabilityTheory.kernel.IsMarkovKernel.comp

instance IsFiniteKernel.comp (η : kernel γ β) [IsFiniteKernel η] (κ : kernel β α)
    [IsFiniteKernel κ] : IsFiniteKernel (η ∘ₖ κ) := by rw [comp_eq_fst_compProd]; infer_instance
#align probability_theory.kernel.is_finite_kernel.comp ProbabilityTheory.kernel.IsFiniteKernel.comp

instance IsSFiniteKernel.comp (η : kernel γ β) [IsSFiniteKernel η] (κ : kernel β α)
    [IsSFiniteKernel κ] : IsSFiniteKernel (η ∘ₖ κ) := by rw [comp_eq_fst_compProd]; infer_instance
...
theorem comp_assoc {δ : Type*} {mδ : MeasurableSpace δ} (ξ : kernel δ γ) [IsSFiniteKernel ξ]
    (η : kernel γ β) (κ : kernel β α) : ξ ∘ₖ η ∘ₖ κ = ξ ∘ₖ (η ∘ₖ κ) := by
...
theorem deterministic_comp_eq_map (hf : Measurable f) (κ : kernel β α) :
...
theorem comp_deterministic_eq_comap (κ : kernel β α) (hg : Measurable g) :
...
noncomputable def prod (κ : kernel β α) (η : kernel γ α) : kernel (γ × β) α :=
  κ ⊗ₖ swapLeft (prodMkRight η β)
...
theorem prod_apply (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ α) [IsSFiniteKernel η] (a : α)
    {s : Set (γ × β)} (hs : MeasurableSet s) :
    (κ ×ₖ η) a s = ∫⁻ b : β, (η a) {c : γ | (c, b) ∈ s} ∂κ a := by
  simp_rw [prod, compProd_apply _ _ _ hs, swapLeft_apply _ _, prodMkRight_apply, Prod.swap_prod_mk]
#align probability_theory.kernel.prod_apply ProbabilityTheory.kernel.prod_apply

theorem lintegral_prod (κ : kernel β α) [IsSFiniteKernel κ] (η : kernel γ α) [IsSFiniteKernel η]
    (a : α) {g : γ × β → ℝ≥0∞} (hg : Measurable g) :
    ∫⁻ c, g c ∂(κ ×ₖ η) a = ∫⁻ b, ∫⁻ c, g (c, b) ∂η a ∂κ a := by
  simp_rw [prod, lintegral_compProd _ _ _ hg, swapLeft_apply, prodMkRight_apply, Prod.swap_prod_mk]
#align probability_theory.kernel.lintegral_prod ProbabilityTheory.kernel.lintegral_prod

instance IsMarkovKernel.prod (κ : kernel β α) [IsMarkovKernel κ] (η : kernel γ α)
...
instance IsFiniteKernel.prod (κ : kernel β α) [IsFiniteKernel κ] (η : kernel γ α)
...
instance IsSFiniteKernel.prod (κ : kernel β α) (η : kernel γ α) :
